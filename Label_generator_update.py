...
def data_match(file_name, data):
    ...
    jarg_pull = []
    for item in text.split('\n'):
        jargon_find = re.search('<Jargon>[^>]*', item)
        if jargon_find != None:
            jarg_pull = jargon_find.group() + '>'
            #print(jarg_pull)
            #jarg_pull = jarg_pull.append(jargon_find.group() + '>')
            text = text.replace(jarg_pull, '[MASK]')
